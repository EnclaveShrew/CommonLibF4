									...
									if (hasDeathChance) {
										...
										if (result > chance) {
											...
											if (a == p) {
												...
												if (a->GetActorValue(*lasthitpart) == 1) {
													SetRicochetValue(a);
												}
			...
			if (parent) {
				...
				if (!deathMarkExcluded) {
					...
					if (!partFound) {
						...
						if (a != p && !strcmp(EFDBodyPartsName[partFound].c_str(), "Head") && a->GetActorValue(*ActorHeadDeathAvoid) > 1) {
							a->SetActorValue(*ActorHeadDeathAvoid, a->GetActorValue(*ActorHeadDeathAvoid) - 1);
							_MESSAGE("Enemy Head Death Avoid -1");
						} else if (a == p && !strcmp(EFDBodyPartsName[partFound].c_str(), "Head") && a->GetActorValue(*ActorHeadDeathAvoid) > 1) {
							a->SetActorValue(*ActorHeadDeathAvoid, a->GetActorValue(*ActorHeadDeathAvoid) - 1);
							_MESSAGE("Player Head Death Avoid -1");
						}
						//물리데미지 탄에 대해서만 출혈 적용
